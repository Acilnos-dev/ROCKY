...
# Módulo OCR de Grado Militar (Requiere: pip install pytesseract)
# Nota: Debes instalar Tesseract OCR en Windows para la lectura de pantalla.
try:
...
class InterfazRocky(ctk.CTk):
    def __init__(self):
        ...
        # ==========================================
        self.title("ROCKY - Terminal Central Optimizada (GRADO MILITAR)")
        ...
        self.LIMITE_MEMORIA = 10 # Aumentado ligeramente para mejor contexto
        self.cerebro_lock = threading.Lock()
        self.micro_activado = True 
        
        # Variables para Monitoreo Autónomo Avanzado
        ...
        # ==========================================
        # 2. CAJA DE HERRAMIENTAS (Ahora con Visión)
        # ==========================================
        self.mis_funciones = {
            ...
            'capturar_pantalla': self.tool_capturar_pantalla, # NUEVO
            'leer_pantalla': self.tool_leer_pantalla        # NUEVO
        }
        ...
        self.label_titulo.grid(row=0, column=0, pady=(15, 5))

        # Panel de Telemetría Dual (RAM + CPU)
        self.frame_telemetria = ctk.CTkFrame(self, fg_color="transparent")
        self.frame_telemetria.grid(row=1, column=0, pady=(0, 10))
        ...
        self.caja_chat.grid(row=2, column=0, padx=15, pady=(5, 10), sticky="nsew")

        mensaje_bienvenida = "========================================\n" \
                             " [ROCKY] Sistema Central Iniciado...\n" \
                             " Motor Neuronal: Llama 3.2:3b\n" \
                             " Arsenal: 8 Herramientas (Visión OCR Activada)\n" \
                             " Telemetría Dual Autónoma: ON\n" \
                             " Optimización Estricta de Memoria: ON\n" \
                             "========================================\n\n"
        self.escribir_en_pantalla(mensaje_bienvenida, nueva_linea=False)

        # Marco Inferior
        self.frame_inferior = ctk.CTkFrame(self, fg_color="transparent")
        self.frame_inferior.grid(row=3, column=0, padx=15, pady=(0, 15), sticky="ew")
    ...
    def precalentar_cerebro(self):
        self.escribir_en_pantalla("[SISTEMA]: Inicializando enlaces sinápticos Llama 3.2:3b...")
        try:
            ollama.chat(
                model='llama3.2:3b',
    ...
    def tool_enviar_whatsapp(self, mensaje: str) -> str:
        numero_destino = "+34600000000" # ¡CAMBIA ESTO POR TU NÚMERO DE PRUEBAS!
    ...
    def tool_capturar_pantalla(self, nombre_archivo="captura_rocky") -> str:
        try:
            ...
            os.startfile(ruta) # Abre la imagen para que la veas
    ...
    def tool_leer_pantalla(self) -> str:
        ...
        try:
            ...
            texto_extraido = pytesseract.image_to_string(foto, lang='spa') # Busca texto en español
            if not texto_extraido.strip():
                return "Sensores activos, pero no detecto ningún texto legible en la pantalla actual."
            return f"He escaneado la pantalla. El texto detectado es:\n{texto_extraido[:800]}..." # Limitado para no saturar memoria
        except Exception as e:
            return f"Fallo en el módulo de lectura OCR: Asegúrate de tener Tesseract-OCR instalado en Windows. Detalle: {e}"

    # ==========================================
    # 6. EL CEREBRO AUTÓNOMO (Optimizando RAM)
    # ==========================================
    def pensar_y_hablar(self, texto_usuario):
        ...
        # Prompt del sistema mejorado para incorporar sus nuevas habilidades
        ...
        sistema = {
            ...
            'content': 'Eres ROCKY, un ingeniero alienígena de Erid. Eres lógico y eficiente. '
                       ...
                       'Usa las herramientas SOLO cuando se te pida una acción explícita (ej. "abre discord", "qué hora es"). '
                       'Responde de forma breve, técnica y con el toque curioso de un eridiano.'
        }
        
        try:
            # LLAMADA PRINCIPAL: Mantiene tu optimización de RAM intacta
            respuesta = ollama.chat(
                model='llama3.2:3b', 
                messages=[sistema] + self.historial_mensajes,
                tools=self.herramientas_para_ia, 
                stream=False,
                keep_alive=0,  # Expulsa el modelo de la RAM al terminar
                options={
                    "num_ctx": 2048  # Ventana de contexto reducida
                }
            )
